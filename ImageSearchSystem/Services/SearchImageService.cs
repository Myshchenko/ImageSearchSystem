...
namespace ImageSearchSystem.Services
{
    ...
    public class SearchImageService : ISearchImageService
    {
        ...
        public SearchImageService(IConfiguration configuration)
        {
        ...
        {
            var pathToDirectoryWithImages = Configuration.GetSection("PathToDirectoryWithImages").Value;

            if (string.IsNullOrEmpty(pathToDirectoryWithImages))
            {
                throw new Exceptions.ConfigurationException("Value was not found.");
            }

            return Directory.GetFiles(pathToDirectoryWithImages, "*", SearchOption.AllDirectories);
        }
    }
}
